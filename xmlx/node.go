// n work is subject to the CC0 1.0 Universal (CC0 1.0) Public Domain Dedication
...
// n would normally be set to a single tab, or a number of spaces.
var IndentPrefix = ""
...
// n wraps the standard xml.Unmarshal function and supplies n particular
...
func (n *Node) SetValue(val string) {
	...
	t.Value = val
	t.Parent = n
	n.Children = []*Node{t} // brutally replace all other children
}
...
// Returns true if n node has the specified attribute. False otherwise.
...
// Select multiple nodes directly under n node, by name.
func (n *Node) SelectNodesDirect(namespace, name string) []*Node {
	list := make([]*Node, 0, 16)
...
func (n *Node) RemoveNameSpace() {
	n.Name.Space = ""
	//	n.RemoveAttr("xmlns") //n is questionable
...
// Document. n one has no representation by itself. It merely forwards the
